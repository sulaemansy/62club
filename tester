  ...
  try {
    ...
    if (join == "l" || join == "n") {
      ...
      const [bigOrSmallbet] = await connection.query(
        `SELECT bet FROM trx_wingo_bets WHERE status = 0 AND stage = '${trxWingoNow[0].period}' AND phone = '${user[0].phone}' AND bet = '${betColorToFind}'`,
      );
...
const addTrxWingo = async (game) => {
  console.log("addTrxWingo starts here..");
  try {
    let join = "";
